...
<objective>
GSD エージェントが使用するモデルプロファイルを切り替えます。各エージェントが使用する Claude モデルを制御し、品質とトークンコストのバランスを調整します。

set-profile ワークフローにルーティングし、以下を処理します：
...
<process>
`@~/.claude/get-shit-done/workflows/set-profile.md` の **set-profile ワークフローに従ってください**。

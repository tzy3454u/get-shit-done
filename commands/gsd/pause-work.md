...
<objective>
セッション間で完全な作業状態を保持するための `.continue-here.md` 引き継ぎファイルを作成する。

pause-work ワークフローにルーティングし、以下を処理する：
- 最近のファイルから現在のフェーズを検出
...
<context>
状態とフェーズの進捗はワークフロー内で対象ファイルの読み取りにより収集される。
</context>

<process>
`@~/.claude/get-shit-done/workflows/pause-work.md` の **pause-work ワークフローに従うこと**。

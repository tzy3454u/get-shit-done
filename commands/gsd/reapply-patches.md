...
<purpose>
GSD アップデートによりファイルが消去・再インストールされた後、ユーザーが以前保存したローカル変更を新しいバージョンにマージします。上流ファイルも変更されたケースに対応するためインテリジェントな比較を使用します。
</purpose>
...
パッチディレクトリから `backup-meta.json` を読み取ります。
...
変更した後に /gsd:update を実行すると自動的に保存されます。
```
終了します。
...
5. **ステータスを報告：**
   ...
   - `Skipped` — 変更は既に上流に含まれています
   - `Conflict` — ユーザーが解決を選択しました

## ステップ 4: マニフェストの更新

再適用後、将来のアップデートでこれらをユーザーの変更として正しく検出できるようにファイルマニフェストを再生成します：

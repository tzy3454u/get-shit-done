...
<objective>
並列の gsd-codebase-mapper エージェントを使用して既存のコードベースを分析し、構造化されたコードベースドキュメントを生成する。

各マッパーエージェントは担当領域を調査し、`.planning/codebase/` に**ドキュメントを直接書き込む**。オーケストレーターは確認のみを受け取り、コンテキスト使用量を最小限に抑える。

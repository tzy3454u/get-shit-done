...
.planning/phases/にフェーズが存在することを確認すること
```
終了。
...
このコマンドは完了したフェーズに対して動作する。先に/gsd:execute-phaseを実行すること。
```
終了。
...
{それ以外:}
**すべてのテストが合格!** フェーズ${phase_number}は完全にテスト済み。

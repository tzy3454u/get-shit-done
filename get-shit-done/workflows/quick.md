...
JSONをパース: `planner_model`, `executor_model`, `checker_model`, `verifier_model`, `commit_docs`, `quick_id`, `slug`, `date`, `timestamp`, `quick_dir`, `task_dir`, `roadmap_exists`, `planning_exists`。
...
ユーザーに報告:
```
クイックタスク ${quick_id} を作成中: ${DESCRIPTION}
ディレクトリ: ${QUICK_DIR}
```
...
標準的なコンテキストテンプレート構造を使用して`${QUICK_DIR}/${quick_id}-CONTEXT.md`を書き込む:
...
注: クイックタスクのCONTEXT.mdは`<code_context>`と`<deferred>`セクションを省略（コードベーススカウティングなし、フェーズスコープへの延期なし）。簡潔に保つ。`<canonical_refs>`セクションは外部ドキュメントが参照された場合に含める — 外部ドキュメントが該当しない場合のみ省略する。

報告: `コンテキストを記録: ${QUICK_DIR}/${quick_id}-CONTEXT.md`
...
${DISCUSS_MODE ? '- ' + QUICK_DIR + '/' + quick_id + '-CONTEXT.md (ユーザーの決定 — 確定済み、再検討しない)' : ''}
</files_to_read>
...
<output>
計画の書き込み先: ${QUICK_DIR}/${quick_id}-PLAN.md
...
プランナーが返った後:
1. `${QUICK_DIR}/${quick_id}-PLAN.md`に計画が存在することを確認
2. 計画数を抽出（クイックタスクでは通常1）
3. 報告: "計画を作成: ${QUICK_DIR}/${quick_id}-PLAN.md"

計画が見つからない場合、エラー: "プランナーが${quick_id}-PLAN.mdの作成に失敗"
...
<files_to_read>
- ${QUICK_DIR}/${quick_id}-PLAN.md (検証する計画)
</files_to_read>
...
<files_to_read>
- ${QUICK_DIR}/${quick_id}-PLAN.md (既存の計画)
</files_to_read>
...
Task(
  prompt="
クイックタスク ${quick_id} を実行。

<files_to_read>
- ${QUICK_DIR}/${quick_id}-PLAN.md (計画)
- .planning/STATE.md (プロジェクト状態)
...
- 計画のすべてのタスクを実行
- 各タスクをアトミックにコミット
- サマリーを作成: ${QUICK_DIR}/${quick_id}-SUMMARY.md
...
エクゼキューターが返った後:
1. `${QUICK_DIR}/${quick_id}-SUMMARY.md`にサマリーが存在することを確認
...
サマリーが見つからない場合、エラー: "エクゼキューターが${quick_id}-SUMMARY.mdの作成に失敗"
...
must_havesを実際のコードベースに対してチェック。${QUICK_DIR}/${quick_id}-VERIFICATION.mdにVERIFICATION.mdを作成。",
...
- `.planning/STATE.md`
- `$DISCUSS_MODE`でコンテキストファイルが存在する場合: `${QUICK_DIR}/${quick_id}-CONTEXT.md`
- `$FULL_MODE`で検証ファイルが存在する場合: `${QUICK_DIR}/${quick_id}-VERIFICATION.md`
...
クイックタスク ${quick_id}: ${DESCRIPTION}

サマリー: ${QUICK_DIR}/${quick_id}-SUMMARY.md
検証: ${QUICK_DIR}/${quick_id}-VERIFICATION.md (${VERIFICATION_STATUS})
コミット: ${commit_hash}
...
- [ ] クイックIDを生成（YYMMDD-xxx形式、2s Base36精度）
- [ ] `.planning/quick/YYMMDD-xxx-slug/`にディレクトリを作成
- [ ] (--discuss) 曖昧な点を特定して提示、決定を`${quick_id}-CONTEXT.md`に記録
- [ ] `${quick_id}-PLAN.md`がプランナーにより作成（--discussの場合はCONTEXT.mdの決定を遵守）
- [ ] (--full) プランチェッカーが計画を検証、修正ループは最大2回
- [ ] `${quick_id}-SUMMARY.md`がエクゼキューターにより作成
- [ ] (--full) `${quick_id}-VERIFICATION.md`がベリファイアにより作成

...
1回の呼び出しですべてのコンテキストを読み込む（オーケストレーターのコンテキストを最小化するためパスのみ）：
...
**ファイルパス（<files_to_read>ブロック用）：** `state_path`, `roadmap_path`, `requirements_path`, `context_path`, `research_path`, `verification_path`, `uat_path`。ファイルが存在しない場合はnull。
...
**initからの既存アーティファクト：** `has_research`, `has_plans`, `plan_count`。
...
表示：`最大イテレーション数に達しました。{N}個の問題が残っています：` ＋ 問題リスト
...
- **GAPS FOUND / VERIFICATION FAILED** → 結果を表示し、チェーンを停止：
  ```
  自動進行停止：実行にはレビューが必要です。

  上記の出力を確認し、手動で続行してください：

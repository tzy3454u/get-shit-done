<purpose>
未開始の将来フェーズをプロジェクトロードマップから削除し、そのディレクトリを削除し、後続のすべてのフェーズを再番号付けしてクリーンな連番を維持し、変更をコミットする。gitコミットが削除の履歴記録として機能する。
</purpose>
...
STATE.mdとROADMAP.mdの内容も読み込んで、現在の位置を解析する。
</step>

<step name="validate_future_phase">
対象が将来のフェーズ（未開始）であることを確認する：
...
将来のフェーズのみ削除可能:
- 現在のフェーズ: {current}
- フェーズ {target} は現在または完了済み

現在の作業を中断するには、代わりに /gsd:pause-work を使用すること。
```
...
**削除操作全体をgsd-toolsに委譲する：**

```bash
RESULT=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" phase remove "${target}")
```

フェーズに実行済みプラン（SUMMARY.mdファイル）がある場合、gsd-toolsはエラーを返す。ユーザーが確認した場合のみ`--force`を使用すること：
...
- STATE.mdに「削除されたフェーズ」のメモを追加しない — gitコミットが記録
- 完了済みフェーズディレクトリを変更しない
</anti_patterns>

<success_criteria>
フェーズ削除は以下の条件を満たした時に完了：
